...
class GastoRetiroController extends ControllerBase {

    private $url = "gasto_retiro.php";
    private $modelo = "models/GastoRetiroModel.php";
    private $table = "gasto_retiro";
    private $id = "id";
    private $pagina = "gasto_retiro";

    //Accion index
    public function gasto_retiro() {
        //Incluye el modelo que corresponde
        require $this->modelo;
        require "models/ModuloModel.php";

        //Creamos una instancia de nuestro "modelo"
        $GastoRetiro = new GastoRetiroModel();
        $modulo = new ModuloModel();

        //incluimos las variables globales del config
        $config = Config::singleton();

        //traemos todos los datos del gasto_retiro logueado
        ...
        //determina que consulta fue la que realizo en gasto_retiro en la busqueda
        ...
        if ($_GET) {
            ...
            if ($_GET["busqueda"]) {
                ...
                foreach ($_GET as $clave => $valor) {
                    if (($clave != "busqueda" ) && ($clave != "controlador" ) && ($clave != "accion" ) && ($clave != "page" )&& ($clave != "max" )&& ($clave != "item" )&& ($clave != "fecha_inicio" )&& ($clave != "fecha_fin" )) {
                        if ($valor) {
            ...
            $arrayRegistro = $GastoRetiro->estructuraGastoRetiro();
        }
        ...
        if($arrayRegistro["fecha_inicio"]=="")
        {
            $arrayRegistro["fecha_inicio"] = date("Y-m-")."01";
        }
        
        if($arrayRegistro["fecha_fin"]=="")
        {
            $arrayRegistro["fecha_fin"] = date("Y-m-d");
        }
        
        if($busqueda=="")
        {
            $busqueda = $busqueda . " WHERE $this->table.fecha_crear BETWEEN '" . $arrayRegistro["fecha_inicio"] . " 00:00:00' AND '" . $arrayRegistro["fecha_fin"] . " 21:59:59'";
        }
        else
        {
            $busqueda = $busqueda . " AND  $this->table.fecha_crear BETWEEN '" . $arrayRegistro["fecha_inicio"] . " 00:00:00' AND '" . $arrayRegistro["fecha_fin"] . " 21:59:59'";
        }
        ...
        $paginador->param = "&controlador=GastoRetiro&accion=gasto_retiro".$parametros_busqueda;
        ...
        $vars['gastos'] = $GastoRetiro->ejecutaSqlArray('SELECT SUM(costo) AS total FROM gasto_retiro WHERE tipo IN ("Gastos generales ","Nomina","Mantenimiento","Colmedica","Colsubsidio","Seguridad social","Servicios","Impuestos","Trasporte")');
        $vars['retiros'] = $GastoRetiro->ejecutaSqlArray('SELECT SUM(costo) AS total FROM gasto_retiro WHERE tipo IN ("Retiros cuenta corriente Maria Barbosa","Retiros cuenta corriente UEFM")');
    ...
    public function nuevoRegistro() {
        ...
        //Creamos el array para el formulario del gasto_retiro
        $arrayRegistro = $_POST;
        ...
        if (in_array("1", $arrayPermiso) == true || in_array("2", $arrayPermiso) == true) {
            ...
            $id = $GastoRetiro->newGastoRetiro($_POST);
            $arrayRegistro = $GastoRetiro->consultGastoRetiro($id);
    ...
    public function editarRegistro() {
        ...
        if ($GastoRetiro->confirmGastoRetiro($id)) {            
            $arrayRegistro = $GastoRetiro->consultGastoRetiro($id);
            if (!isset($_GET["id"]))
            {
                if (in_array("1", $arrayPermiso)==true||in_array("2", $arrayPermiso)==true) {
                    ...
                    $GastoRetiro->modGastoRetiro($_POST);                
                    $arrayRegistro = $GastoRetiro->consultGastoRetiro($id);
                    $vars['alerta'] = 2;
                }
                else
            ...
            $formXhtml->location("index.php?controlador=GastoRetiro&accion=gasto_retiro");
        }
