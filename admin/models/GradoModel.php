...
class GradoModel extends ModelBase {
     ...
     */
    public function listGrado( $busqueda = "WHERE 1" , $order = "false" ) {
     ...
     * @return bool 
     */
    public function confirmGrado( $id ) {
        settype($id, "integer");
        //realizamos la consulta de todos los grados
     ...
     * @return bool 
     */
    public function confirmGradoWhere( $where ) {
     ...
     * @return $id int
     */
    public function newGrado( $array ) {
     ...
     * @return $id int
     */
    public function modGrado( $array ) {
        ...
        if( $this->confirmGrado($array[$this->idTable]) )        
        {
     ...
     * @return $grado array
     */
    public function consultGrado( $id ) {
        //realizamos la consulta de todos los grados
        $consulta = $this->db->prepare('SELECT * FROM '.$this->table.' WHERE '.$this->idTable.' = "'.$id.'"');                 
        $consulta->execute();
    ...
    public function consultGradoWhere( $where ) {
        //realizamos la consulta de todos los grados        
        $consulta = $this->db->prepare('SELECT * FROM '.$this->table.' WHERE '.$where);                 
        $consulta->execute();
        $grado = $consulta->fetch(PDO::FETCH_ASSOC);        
     ...
     * @param $id int 
     */
    public function estructuraGrado() {
