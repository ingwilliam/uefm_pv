...
class PensionModel extends ModelBase {

    private $table = "pension";
    private $idTable = "id";
     ...
     */
    public function listPension( $busqueda = "WHERE 1" , $order = "false" ) {
     ...
     * @return bool 
     */
    public function confirmPension( $id ) {
        settype($id, "integer");
        //realizamos la consulta de todos los grados
     ...
     * @return bool 
     */
    public function confirmPensionWhere( $where ) {
     ...
     * @return $id int
     */
    public function newPension( $array ) {
     ...
     * @return $id int
     */
    public function modPension( $array ) {
        ...
        if( $this->confirmPension($array[$this->idTable]) )        
        {
     ...
     * @return $grado array
     */
    public function consultPension( $id ) {
        //realizamos la consulta de todos los grados
        $consulta = $this->db->prepare('SELECT * FROM '.$this->table.' WHERE '.$this->idTable.' = "'.$id.'"');                 
        $consulta->execute();
    ...
    public function consultPensionWhere( $where ) {
        //realizamos la consulta de todos los grados        
        $consulta = $this->db->prepare('SELECT * FROM '.$this->table.' WHERE '.$where);                 
        $consulta->execute();
        $grado = $consulta->fetch(PDO::FETCH_ASSOC);        
     ...
     * @param $id int 
     */
    public function estructuraPension() {
