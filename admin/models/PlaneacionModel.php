...
class PlaneacionModel extends ModelBase {
     ...
     */
    public function listPlaneacion( $busqueda = "WHERE 1" , $order = "false" ) {
     ...
     * @param $id int 
     */
    public function delPlaneacion( $id ) {
        settype($id, "integer");
     ...
     * @return bool 
     */
    public function confirmPlaneacion( $id ) {
        settype($id, "integer");
        //realizamos la consulta de todos los grados
     ...
     * @return bool 
     */
    public function confirmPlaneacionWhere( $where ) {
     ...
     * @return $id int
     */
    public function newPlaneacion( $array ) {
     ...
     * @return $id int
     */
    public function modPlaneacion( $array ) {
        ...
        if( $this->confirmPlaneacion($array[$this->idTable]) )        
        {
     ...
     * @return $grado array
     */
    public function consultPlaneacion( $id ) {
        //realizamos la consulta de todos los grados
        $consulta = $this->db->prepare('SELECT * FROM '.$this->table.' WHERE '.$this->idTable.' = "'.$id.'"');                 
        $consulta->execute();
    ...
    public function consultPlaneacionWhere( $where ) {
        //realizamos la consulta de todos los grados        
        $consulta = $this->db->prepare('SELECT * FROM '.$this->table.' WHERE '.$where);                 
        $consulta->execute();
        $grado = $consulta->fetch(PDO::FETCH_ASSOC);        
     ...
     * @param $id int 
     */
    public function estructuraPlaneacion() {
